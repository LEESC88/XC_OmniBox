...
| 2026-09-17 | **v1.0.0 (全套免费离线 AI 矩阵落地)** | 模块五：🤖 AI 智能工坊 (Edge AI Suite) 全套交付 | 1. **100% 免费 / 无限次使用 / 零 API 成本 / 零云端 GPU 消耗**：坚守纯前端浏览器端本地边缘算力 (Edge AI / WebAssembly / ONNX)，无需申请任何外部 API Key，无任何调用次数限制，彻底保障用户隐私绝不外泄。<br>2. **AI 发丝级复杂背景智能抠图**：引入量化轻量级 ISNet 视觉分割模型与自适应色彩边缘双引擎，支持复杂生活照、人像发丝、电商静物与宠物羽毛精准抠出；支持 6 种底色预设（无损透明底、纯白底、经典证件蓝、结婚证红、极简灰、暗黑）以及拖拽对比滑块实时查看；支持【一键转入 6 寸证件照排版】无缝联动模块四！<br>3. **AI 离线 OCR 文字提取**：集成 Tesseract.js WebAssembly 引擎，支持中文简体、英文、繁体中文、日文等多语言识别；配备实时解析进度、置信度统计、字数/行数统计、内嵌可编辑文本区，以及一键复制全文与 TXT 文本导出。<br>4. **AI 模糊图片超清修复与超分辨率**：纯客户端自适应高阶双三次样条超分辨率（2x / 4x 放大），配合拉普拉斯高频微分锐化卷积核与自适应阈值边缘门控，去除雾感并消除 JPEG 噪点，配备原图 vs 超清图左右无级滑动对比工作台与超清 PNG 导出。<br>5. **全站五大矩阵大一统**：贯通「文档、图片、音频、日常、AI工坊」全链路体验，自适应椰林与暗夜沉浸主题。 |
